...
class ChessGame:
    ...
    def load_pieces(self):
        ...
        for piece in ['p', 'r', 'n', 'b', 'q', 'k']:
            try:
                # Load black piece
                black_piece_path = os.path.join(pieces_dir, f"{piece}.png")
                print(f"Attempting to load black piece: {black_piece_path}")
                img = pygame.image.load(black_piece_path)
                self.pieces[piece] = pygame.transform.scale(img, (self.square_size, self.square_size))

                # Load white piece
                white_piece_path = os.path.join(pieces_dir, f"w{piece}.png")
                print(f"Attempting to load white piece: {white_piece_path}")
                img = pygame.image.load(white_piece_path)
                self.pieces[piece.upper()] = pygame.transform.scale(img, (self.square_size, self.square_size))
            except pygame.error as e:
                print(f"Error loading piece {piece}: {e}")
